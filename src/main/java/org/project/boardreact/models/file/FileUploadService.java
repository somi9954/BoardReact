    ...
    public List<FileInfo> upload(MultipartFile[] files, String gid, String location) {
        ...
        for (MultipartFile file : files) {
            ...
            try {
                ...
                // 썸네일 생성 처리 시작
                if (contentType.indexOf("image") != -1) { // 이미지 형식 파일
                    String thumbPath = infoService.getThumbPath(item.getSeq(), item.getExtension(), width, height);
                    String thumbUrl = infoService.getThumbUrl(item.getSeq(), item.getExtension(), width, height);

                    item.setThumbPath(Arrays.toString(new String[] { thumbPath }));
                    item.setThumbUrl(Arrays.toString(new String[] { thumbUrl }));
